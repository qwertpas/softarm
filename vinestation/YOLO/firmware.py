...
class XMixedReplaceResponse(Response):
    ...
    def send_frame(self, frame: Union[str, bytes] = "") -> None:
        # Combine header and boundary to reduce packets
        header = (
            f"{self._boundary}\r\n"
            f"Content-Type: {self._frame_content_type}\r\n\r\n"
        )
        self._send_bytes(self._request.connection, header.encode("utf-8"))
        
        encoded_frame = frame if isinstance(frame, bytes) else frame.encode("utf-8")
...
async def send_stream_frames():
    while True:
        ...
        # await asyncio.sleep(0) # Max FPS
        await asyncio.sleep(1/20.0) # Max FPS
